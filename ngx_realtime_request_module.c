...
#define REALTIME_REQUEST_MODULE_VERSION "0.6"
...
static ngx_int_t ngx_http_realtime_handler(ngx_http_request_t *r)
{
    ...
    int m_day, m_hour, m_minute, m_seconds;

    m_day = (time->sec - rrmf->startup) / 24 / 60 / 60;
    m_hour = (time->sec - rrmf->startup - m_day * 24 * 60 * 60) / 3600;
    m_minute = (time->sec - rrmf->startup - m_day * 24 * 60 * 60 - m_hour * 3600) / 60;
    m_seconds = time->sec - rrmf->startup - m_day * 24 * 60 * 60 - m_hour * 3600 - m_minute * 60;

    char tmp_startup[50] = {0};
    char tmp_tablehead[255] = {0};
    char tmp_tablestyle[255] = {0};

    sprintf(tmp_startup, "运行时间:%04d天%02d时%02d分%02d秒 当前版本:" REALTIME_REQUEST_MODULE_VERSION "<br>", m_day, m_hour, m_minute, m_seconds);
    strcpy(tmp_tablestyle, "<style>.tablelist{border:1px solid;border-collapse:collapse}.tablelist td{border:1px solid;text-align:right}tr:nth-child(even){background:#43d3bb}tr:nth-child(odd){background:#7d9650}</style>");
    strcpy(tmp_tablehead, "<tr><td>域名</td><td>请求量</td><td>接收字节(MB)</td><td>发送字节(MB)</td><td>upstream_recv</td><td>响应20x状态码次数</td><td>响应30x状态码次数</td><td>响应40x状态码次数</td><td>响应50x状态码次数</td></tr>");

    size += strlen(tmp_startup);
    size += strlen(tmp_tablestyle);
    size += strlen("<table class=\"tablelist\"></table>");
    size += strlen(tmp_tablehead);
    ...
    for (i = 0; i < rrmf->srv_list->nalloc; i++)
    {
        ...
        size += strlen("<tr>");
        size += strlen("<td>");
        size += (*ptr)->server_name.len;
        size += strlen("</td>");
        size += (strlen("td") + NGX_ATOMIC_T_LEN + strlen("</td>")) * 8;
        size += strlen("</tr>");
        ptr++;
    }
    ...
    b->last = ngx_cpymem(b->last, tmp_startup, strlen(tmp_startup));

    b->last = ngx_cpymem(b->last, tmp_tablestyle, strlen(tmp_tablestyle));
    b->last = ngx_cpymem(b->last, "<table class=\"tablelist\">", sizeof("<table calss=\"tablelist\">") - 1);
    b->last = ngx_cpymem(b->last, tmp_tablehead, strlen(tmp_tablehead));
    ...
    for (i = 0; i < rrmf->srv_list->nalloc; i++)
    {
        ...
        b->last = ngx_sprintf(b->last, "<tr><td>%s</td><td>%ul</td><td>%.2f</td><td>%.2f</td><td>%ul</td><td>%ul</td><td>%ul</td><td>%ul</td><td>%ul</td></tr>",
                              (*ptr)->server_name.data,
                              (*ptr)->request,
                              (float)((*ptr)->recv) / 1024 / 1024,
                              (float)((*ptr)->sent) / 1024 / 1024,
                              (*ptr)->upstream_recv,
                              (*ptr)->request_20x,
                              (*ptr)->request_30x,
                              (*ptr)->request_40x,
